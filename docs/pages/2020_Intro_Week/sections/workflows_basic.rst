...
This is not a significant issue when running these simple examples, but when you start running workflows that e.g. use a more computationally demanding code, this can become a real problem.
...
* the **exit codes** of the work chain, specified using the ``spec.exit_code()`` method. 
  These are important for communicating known failure modes to the user, and potentially use this information to recover from these failures. 
  The advanced section on workflows discusses these concepts in more detail.
...
This is done in the ``validate_result`` method, which simply extracts the result from the context and returns the exit code set up in the ``define()`` method.
The advanced workflow section will explain the use of exit codes in more detail.
...
.. code:: python
    ...
    initial_structure = load_node(pk=4194)
...
.. code-block:: ipython
    ...
    Out[3]:
    [<StructureData: uuid: 252e63b1-cd0d-4a44-a9df-6a542d82f067 (pk: 4197)>,
    <StructureData: uuid: b863bbbe-3e5a-4248-91ac-c0b6d5147ba2 (pk: 4200)>,
    <StructureData: uuid: e89f5465-0a28-4347-b8d3-58c383fad075 (pk: 4203)>,
    <StructureData: uuid: 754683d7-c45d-4428-b34f-c40ee418fe34 (pk: 4206)>,
    <StructureData: uuid: 77de654b-948e-4f3e-b90b-31a706d6be79 (pk: 4209)>]
...
In the ``eos_wf.py`` script, a work function has been implemented that does this for a range of 5 scaling factors:

.. literalinclude:: include/snippets/workflows_eos.py
    :language: python

Let's open up a ``verdi shell`` and run the equation of state workflow. First, load the silicon structure you imported earlier using its PK:
...
.. code-block:: ipython
    ...
    In [4]: from eos_wf import run_eos_wf
...
.. code-block:: ipython
    ...
    In [5]: result
    Out[5]: 
    {'initial_structure': <StructureData: uuid: c65d0871-6e61-44ac-ad1a-47eeb1b4e755 (pk: 234)>,
    'eos': <Dict: uuid: f4a83444-bdac-4c9a-9a93-245a234e1b57 (pk: 475)>}

We can see that the work function returns the initial silicon structure, as well as a ``Dict`` node with the results for the equation of state.
Let's have a look at the contents of this node:

.. code-block:: ipython

    In [6]: result['eos'].get_dict()
    Out[6]: 
...
Similar to the simple arithmetic work function above, running the ``eos_wf`` work function means that the Python interpreted will be blocked during the whole workflow.
In this case, this involves the time in which the calculations are submitted, and the actual time needed by Quantum ESPRESSO to perform the calculation and the time taken to retrieve the results.
...
Download the following tarball and extract the contents:
...
Next use ``pip`` to install the package:

.. code-block:: bash

    $ pip install -e eos

You can now use ``pip list`` to see if the ``eos`` package has been installed successfully.
Once the ``eos`` package has been installed, it is **very important** to restart the daemon, so it can successfully find the new package and execute the work chain therein:

.. code-block:: bash

    $ verdi daemon restart

Now that the ``eos`` package is installed, and the daemon has been restarted, it is time to *submit* the ``EquationOfState`` work chain from the ``verdi shell``:

.. code-block:: ipython

    In [1]: from eos.workchain import EquationOfState
...
Once the work chain is ``Finished``, you can try to use the skills you learned from the "Working with data" section to find the output ``Dict`` from the ``EquationOfState`` work chain, and once again use the ``plot_eos`` function to plot the equation of state.
